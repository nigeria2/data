"""
Downloads every per-senatorial-district PDF result sheet linked from INEC's
2019 Senate results index page into ./senate_2019_pdfs/{State}_{Name}.pdf,
for later scraping.

Source index: https://www.inecnigeria.org/2019-senatorial-district-elections-result/
Same page structure as the House of Representatives index (see
fetch_house_2019_pdfs.py): a series of state accordions (Elementor tabs) in
strict alphabetical order (36 states + FCT between Enugu and Gombe), each
listing PDFs. Most states link 3 PDFs (one per senatorial district); FCT
links 3 sub-area PDFs for its single seat (two area councils plus a combined
"FCT Senatorial District" file) and Imo links only 2 -- both are as found on
the live page, not a scraping bug.
...
Usage:
    python fetch_senate_2019_pdfs.py
"""
...
INDEX_URL = "https://www.inecnigeria.org/2019-senatorial-district-elections-result/"
SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = SCRIPT_DIR / "senate_2019_pdfs"
...
def fetch_index() -> list[tuple[str, str, str]]:
    """Returns (state, district_label, pdf_url) for every linked PDF."""
...
def main() -> int:
    ...
    entries = fetch_index()
    print(f"Found {len(entries)} senatorial district PDF links across {len(STATE_ORDER)} states.")
...
def write_report(results: list[dict]) -> None:
    ...
    lines = ["# Senate 2019 PDF download report", ""]
    lines.append(f"{len(ok)}/{len(results)} downloaded, {len(bad)} failed.")
    lines.append("")
    if bad:
        lines.append("## Failures")
        lines.append("")
        lines.append("| State | District | URL | Reason |")
        lines.append("| --- | --- | --- | --- |")
        for r in bad:
    ...
    lines.append("")
    lines.append("| State | District | File |")
    lines.append("| --- | --- | --- |")
    for r in results:
